...
const TeamCard = ({ member, priority = false }) => (
    <div className="group relative">
        <div className="relative h-64 w-full overflow-hidden rounded-xl bg-zinc-100 dark:bg-white/5 mb-4 shadow-sm border border-zinc-200 dark:border-white/5">
            {member.image ? (
                <Image
                    src={member.image}
                    alt={member.name}
            ...
            <main className="max-w-5xl mx-auto px-6 pt-32 pb-24">
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-32">
                    {teamMembers.map((member, i) => (
                        <TeamCard key={i} member={member} priority={i < 2} />
                    ))}
                </section>
